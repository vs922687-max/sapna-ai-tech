      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 pt-14 sm:px-6">
        ...
        <div className="mt-10">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {CORE_TOOLS.map((tool) => <ToolCard key={tool.slug} tool={tool} />)}
          </div>
        </div>
        ...
        <div className="mt-16 border-t border-border/60 pt-12" id="office-tools">
          ...
          <h2 className="mt-2 font-display text-2xl font-bold">
            Everything you need to run your office, in one place.
          </h2>
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {OFFICE_TOOLS.map((tool) => <ToolCard key={tool.slug} tool={tool} />)}
          </div>
        </div>
...
function ToolCard({ tool }: { tool: AiTool }) {
  return (
    <Link to={tool.to} className="group relative block overflow-hidden rounded-2xl border border-border/60 bg-card/40 p-6 backdrop-blur-xl transition-all hover:border-primary/40 hover:shadow-glow">
